...
// =========================================================
//  IMPLEMENTACI�N DE LA CLASE CLASIFICADOR
// =========================================================
...
bool Clasificador::Load(const std::string& modelPath, const std::string& scalerPath) {
    ...
    // 2. Cargar Scaler (Media y Desviaci�n est�ndar) si existe
    if (!scalerPath.empty()) {
        ...
        if (fs.isOpened()) {
            ...
            // Asegurar tipos compatibles para operaciones matem�ticas
...
int Clasificador::Predict(const cv::Mat& img) {
    ...
    // 3. Aplicar Normalizaci�n (Scaler) si existe
...
int RunTrain(const TrainSVM::Options& opts) {
    ...
    if (opts.doGridSearch && samples.rows > 1) {
        // grid values (ajusta seg�n necesites)
        ...
        // Entrena modelo final RBF con mejores hiperpar�metros
    ...
    // Si no se pidi� grid-search (opts.doLOO == false), se entrena el SVM polin�mico como antes.
...
// ------------------------- EVALUATE (adaptaci�n compacta de EvaluacionClasificador.cpp) -------------------------
