...
namespace FeatureExtractor {
    ...
    static std::vector<double> local_extractShapeFeatures(const cv::Mat& I_in)
    {
        ...
        // Ahora son 6 (sin EulerNumber)
        std::vector<double> feat(6, 0.0);
        ...
        {
            cv::Mat skel = morphologicalSkeleton(maskR);
        ...
        // Orden igual que antes pero sin EulerNumber
        feat[0] = Circularity;
        feat[1] = Extent;
        feat[2] = Solidity;
        feat[3] = Eccentricity;
    ...
    // (SIN EulerNumber en el vector final)
    //----------------------------------------------------------------------------
    void ExtractColorShapeFeatures(const Mat& I_in, std::vector<double>& feat, std::vector<std::string>& featNames) {
        ...
        double S_median = featColor[1];
        double V_IQR = featColor[2];
        double S_mean = featColor[3];
        double V_mean = featColor[4];
        ...
        double SkelLenNorm = featShape[4];
        double FD5 = featShape[5];

        // Vector final (11 características)
        feat = {
